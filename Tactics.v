...
Proof.
  intros.
  assert (HH: forall l l' : list nat, 
  l = l' -> rev l = rev l'). {intros.  rewrite H0. reflexivity. }
  apply HH in H. rewrite rev_involutive in H.
  symmetry.
  apply H.
Qed.
...
(* FILL IN HERE 
apply is backward, rewrite is forward.
    [] *)
...
(* H1 and H3 can't hold at the same time*)
...
(* FILL IN HERE 
we do induction to n and do case analtsis for m.
when n = O, m has to be O as well.
when n = S n', m can't be O. we assume m = S m'.
n' = m' => n' = m' => S m' = S n' => S n = S m.
quite easily done. ^_^
*)
...
Theorem eqb_id_true : forall x y,
  eqb_id x y = true -> x = y.
Proof.
  intros [m] [n]. simpl. intros H. Check eqb_true. 
...
Proof.
  intros X Y l.
  induction l.
  - intros. simpl. injection H as H1 H2.
    rewrite <- H1. rewrite <- H2. reflexivity.
  - intros. simpl in H. destruct x as [m n]. 
    destruct (split l) as [llx lly]. injection H as H1 H2. 
    rewrite <- H1. rewrite <- H2. simpl. 
    assert ( lemma : combine llx lly = l). { apply IHl. reflexivity. } 
...
Proof.
  ...
  - destruct (f false) eqn:Ef.
    + destruct b.
      * do 3 rewrite Et. reflexivity.
      * rewrite Ef. do 2 rewrite Et. reflexivity.
    + destruct b.
      * do 3 rewrite Et. reflexivity.
      * do 3 rewrite Ef. reflexivity.
  - destruct (f false) eqn:Ef.
    + destruct b.
      * rewrite Et. rewrite Ef. rewrite Et. reflexivity.
      * rewrite Ef. rewrite Et. rewrite Ef. reflexivity.
    + destruct b.
      * rewrite Et. do 2 rewrite Ef. reflexivity.
      * do 3 rewrite Ef. reflexivity.
Qed.
...
(** **** Exercise: 3 stars, advanced, optional (eqb_sym_informal)  
   ...
   Proof: *)
   (* FILL IN HERE 
  we do induction to n and try to prove that forall m,
  (n =? m) = (m =? n) => (S n =? m) = (m =? S n)
  we solve it by destruct m and apply the induction hypothesis.
...
Proof.
  ...
  assert (HH: n = p). {apply trans_eq with m. apply H. apply H0. }
  rewrite HH. symmetry. apply eqb_refl.
Qed.  
...
Proof.
  ...
  - intros. simpl. simpl in H. 
    assert (HH: l2 = []). {destruct l2. reflexivity. discriminate. }
    rewrite HH. reflexivity.
  - intros. simpl. 
    destruct l2.
    + simpl. assert (HH: x :: l1 = []). 
      {destruct (x :: l1). reflexivity. discriminate. } 
      rewrite HH. reflexivity.
    + simpl. simpl in H. injection H as H'. apply IHl1 in H'.
      rewrite H'. reflexivity.
Qed.
...
Proof.
  induction l.
  - intros. simpl in H. destruct lf. discriminate. discriminate.
...
Proof. 
  ...
  - simpl. destruct (test x) eqn:E. 
    ...
    + unfold existsb'. simpl. rewrite E. simpl. unfold existsb' in IHl.
