...
(* FILL IN HERE 
  Destruct is used for case analysis, while induction for maths induction.
  The difference is that we can use an additional condition for induction.*)
*)
...
(** **** Exercise: 2 stars, advanced, recommended (plus_comm_informal)  
    ...
    Proof: (* FILL IN HERE 
By induction on n.
First, suppose n = 0. We can easily get 0 + m = m + 0. 
Next, suppose n = S n', given n' + m = m + n',
we need to show S (n' + m) = m + Sn'.
use the condition, we need to prove S(m + n') = m + Sn'.
By Theorem plus_n_Sn, proved. Qed.
*)
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal)  
    ...
    Proof: (* FILL IN HERE 
If n is true, then true = true,
else, n is false, then false = false.
Qed.
    [] *)
*)
...
Proof.
  induction m as [|m' IHm'].
    ...
    - induction n as [|n' IHn'].
      + simpl. rewrite -> mult_0_r. reflexivity.
      + simpl. rewrite <- IHn'. 
        rewrite -> (IHm' (S n')). simpl. 
        rewrite -> (plus_assoc n' m' (n' * m')).
        rewrite -> (plus_assoc m' n' (m' * n')).
...
Proof.
  ...
  - do 3 rewrite mult_0_r. reflexivity.
  - rewrite mult_comm. simpl.
    rewrite (mult_comm n (S p')). simpl.
    rewrite (mult_comm m (S p')). simpl.
    rewrite (plus_swap (n + p' * n) m (p' * m)).
    rewrite (mult_comm p' (n + m)).
    rewrite (mult_comm p' n).
    rewrite (mult_comm p' m).
    rewrite IHp'.
    rewrite (plus_comm n m).
    rewrite <- (plus_assoc m n (n * p' + m * p')).
    rewrite <- (plus_assoc n (n * p') (m * p')).
    reflexivity.
Qed.
...
Proof.
  ...
  induction n as [|n' IHn'].
  - simpl. reflexivity.
  - simpl. rewrite ( bin_to_nat_pres_incr (nat_to_bin n') ).
...
(* FILL IN HERE 
  we have not clearly defined equal function in bin.
*)
...
Compute bin_to_nat (A(A(A(B(A Z))))).
...
Theorem lemma_one : forall (b : bin),
...
Theorem lemma_two : forall (n : nat),
nat_to_bin (n + n) = double_bin (nat_to_bin n).
Proof.
  induction n as [|n' IHn'].
  - simpl. reflexivity.
  - rewrite <- double_plus. simpl. rewrite -> double_plus.
    rewrite -> IHn'. rewrite -> lemma_one. reflexivity.
Qed.

Theorem lemma_three : forall (b : bin),
incr (double_bin b) = B (b).
Proof.
  destruct b as [|b'|b''].
  - simpl. reflexivity.
  - simpl. reflexivity.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. rewrite <- IHb'.
    (* take bin_to_nat b' as a whole*)
    rewrite -> plus_assoc. 
    rewrite <- (plus_n_O (bin_to_nat b' + bin_to_nat b')).
    rewrite -> (lemma_two (bin_to_nat b')).
    reflexivity.
  - simpl. 
    rewrite -> plus_assoc. 
    rewrite <- (plus_n_O (bin_to_nat b'' + bin_to_nat b'')).
    rewrite <- IHb''.
    rewrite -> (lemma_two (bin_to_nat b'')).
    rewrite -> (lemma_three (nat_to_bin (bin_to_nat b''))).
    reflexivity.
Qed.
