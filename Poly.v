...
Actually it is forall X Y : Type
...
filter (fun n => negb(n <=? 7) && evenb n) l.
...
Lemma kk_map_rev : forall (X Y : Type) (f : X -> Y) (l : list X) 
(x : X),
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. rewrite <- IHl. 
    rewrite -> kk_map_rev with X Y f (rev l) x.
    reflexivity.
Qed.
...
(* FILL IN HERE 
  X is Monday. Y is nat, which is the saling of candies today.
  f can guess the saling of candies tomorrow.
*)
...
*)
(* FILL IN HERE 
  it is easy to dothe translation to english, but the formal
  prove is a little bit tricky.
*)
Lemma easier : forall (X : Type) (l : list X), 
...
Lemma small_exe : forall (X : Type) (n : nat) (l :list X),
length l = n -> @nth_error X l n = None.
Proof. 
  intros.
  rewrite <- H. rewrite -> easier. reflexivity.
Qed.
...
Definition mult (n m : cnat) : cnat :=
  fun (X : Type) (f : X -> X) (x : X) => (n X (fun l => (m X f l)) x).
