...
Proof.
  (* WORKED IN CLASS *)
  intros. unfold not. intros. destruct H0. apply H.
Qed.
...
(* FILL IN HERE 
  First we unfold not. Then we need to prove that (P -> False) -> False.
  Destruct the hypothesis and we can get the conclusion.
*)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros. unfold not. intros. destruct H0. apply H. apply H1.
Qed.
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros. unfold not. intros [H1 H2].
  apply H2. apply H1.
Qed.
...
(* FILL IN HERE 
  unfold not, we get P /\ (P -> False) -> False. we then divide the
  hypothesis into P and P -> False. apply both of them and we are done.
*)
...
Proof.
  ...
  - intros H6. apply H2. apply H4. apply H6.
Qed.
...
Proof.
  split.
  - apply mult_eq_0. Check or_example.
  - apply or_example.
Qed.
...
Proof.
  ...
  - intros H. induction l.
    ...
    + simpl in H. destruct H as [H1 | H2].
      ...
      ++ apply IHl in H2. destruct H2 as [x' [Hx' Hx'']].  exists x'.
         split. +++ apply Hx'. +++ simpl. right. apply Hx''.
  - intros [x [Hx1 Hx2]].
    rewrite <- Hx1. apply In_map. apply Hx2.
Qed.
...
Proof.
  ...
  - simpl. split.
    + intros [H1|H2].
      ++ left. left. apply H1.
      ++ apply IHl in H2. rewrite <- or_assoc. right. apply H2.
...
Proof.
  ...
  - simpl. split. 
    ...
    + intros. destruct H0 as [H1|H2]. destruct H as [H3 H4].
      ++ rewrite <- H1. apply H3.
      ++ destruct H as [H3 H4]. rewrite <- IHl in H4.
...
Lemma solve_it : forall {X : Type} (l1 l2 : list X),
...
Proof.
  ...
  - simpl. intros. rewrite IHl1. rewrite (IHl1 [x]).
    rewrite <- (app_assoc X (rev_append l1 [ ]) [x] l2).
    reflexivity.
Qed.
...
Proof.
  ...
  induction x.
  - reflexivity.
  - simpl. rewrite <- IHx. apply solve_it.
Qed.
...
Proof.
  ...
  - rewrite evenb_S. destruct IHn as [k' Hk']. 
    destruct (evenb n).
    + simpl. exists k'. rewrite Hk'. reflexivity. 
    + simpl. exists (S k'). rewrite Hk'. reflexivity.
Qed.
...
Proof.
  ...
  - intros. destruct H as [H1|H2].
    ...
    + rewrite H2. simpl. destruct b1. reflexivity. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - intros. unfold not. intros.
    rewrite H0 in H. rewrite <- eqb_refl in H. discriminate H.
  - intros. unfold not in H. destruct (x =? y) eqn:E.
    + destruct H. Search eqb. apply eqb_true in E. apply E.
    + reflexivity.
Qed.
...
Proof.
  ...
  - generalize dependent l2. induction l1.
    ...
    + intros. destruct l2. 
      ...
      ++ simpl in H0. destruct (eqb x x0) eqn:E.
        +++ apply IHl1 in H0. rewrite H in E. 
  ...
  - generalize dependent l2. induction l1. 
    ...
    + destruct l2.
      ...
      ++ simpl. intros. destruct (eqb x x0) eqn:E.
        +++ apply IHl1. injection H0 as H1 H2. apply H2.
        +++ injection H0 as H1 H2.  
            apply H in H1. rewrite H1 in E. discriminate E.
Qed.
...
Proof.
  intros. unfold not in H0. destruct (H (P x)) as [H1|H2] . 
  - apply H1.
...
Proof.
  ...
  - intros. destruct (H P) as [H1|H2] . 
    + apply H1.
    + unfold not in H2. apply H0. intros. apply H2 in H1.
      destruct H1.
Qed. 
...
Proof.
  ...
  intros.
  apply H with (Q:= False). intros. apply H0 in H1. destruct H1.
Qed. 
...
Proof.
  ...
  unfold de_morgan_not_and_not. unfold not. intros.
...
Proof.
  ...
  intros. apply H. unfold not. intros. destruct H1 as [H2 H3].
  apply H2. intros. apply H3. apply H0. apply H1.
Qed.

Lemma or_exchange: forall P: Prop,
~P \/ P -> P \/ ~P.
Proof.
  intros. destruct H as [H1|H2].
  - right. apply H1.
...
Proof.
  ...
  intros. apply or_exchange. apply H. intros. apply H0.
Qed.
