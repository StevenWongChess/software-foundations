...
Proof.
  intros. inversion H.
  apply evSS_ev. apply H1.
Qed.

Theorem SSSSev__even_extra : forall n,
  even (S (S (S (S n)))) -> even n.
Proof.
  intros. apply ev_inversion in H. destruct H.
  - discriminate H.
  - destruct H as [n' [Hn' E]]. injection Hn'. intros. 
    rewrite <- H in E. apply evSS_ev. apply E.
Qed.
...
Proof.
  ...
  - intros. induction H. 
    ...
    + apply (ev_sum n m). apply IHeven'1. apply IHeven'2.
  - intros. induction H.
    + apply even'_0.
    + assert (Lemma': forall n: nat, S (S n) = n + 2).
      {intros. induction n0. 
       - simpl. reflexivity. 
       - simpl. rewrite IHn0. reflexivity. }
      rewrite Lemma'. apply (even'_sum n 2). 
      ++ apply IHeven.
      ++ apply even'_2.
Qed.
...
Proof.
  intros. induction H0.
  - simpl in H. apply H.
  - simpl in H. apply evSS_ev in H. apply IHeven. apply H.
Qed.
...
Proof.
  ...
  - rewrite (plus_assoc (n+n) m p). 
    rewrite <- (plus_assoc n n m). rewrite (plus_comm n (n+m)).
    rewrite <- (plus_assoc (n+m) n p).
    apply (ev_sum (n+m) (n+p)). 
    + apply H.
    + apply H0.
  - assert (HH: forall n0:nat, even(n0+n0)). 
    {induction n0. 
       - simpl. apply ev_0. 
       - simpl. rewrite <- plus_n_Sm. apply ev_SS. apply IHn0. }
    apply HH.
Qed.
...
Proof.
  intros. inversion H.
  - apply le_n.
  - assert (HH: forall x:nat, x <= S x).
    {destruct x. - apply le_S. apply le_n. - apply le_S. apply le_n. }
    apply (le_trans n (S n) m). apply HH. apply H1.
Qed.
...
Proof.
  ...
  - induction H.
    ...
    + apply (le_trans (S n1) m (S m)).
      ++ apply IHle. ++ apply le_S. apply le_n.
  - induction H.
...
No, since m and n is symmetric. vector space of 101 011 -1-1-2?

    - If we dropped constructor [c4] from the definition of [R],
      would the set of provable propositions change?  Briefly (1
      sentence) explain your answer.
No, since apply c4 is just a combination of the inverse of (c2 and c3).
we can say that c2 c3 and c4 are linear dependent.
...
Proof.
  ...
  - intros. induction H.
    ...
    + simpl in IHR. assert(HH: forall x y:nat, S x =S y -> x=y).
      {intros. inversion H0. reflexivity. }
      apply HH in IHR. rewrite <- plus_n_Sm in IHR.
      apply HH in IHR. apply IHR.
    + rewrite plus_comm. apply IHR.
  - generalize dependent o. generalize dependent n. 
    induction m.
    + induction n.
      ++ intros. simpl in H. rewrite <- H. apply c1.
      ++ intros. simpl in H. rewrite <- H. apply c3. 
...
| q1 l2 : subseq nil l2
| q2 l1 l2 h (H: subseq l1 l2) : subseq l1 (h::l2)
| q3 l1 l2 h (H: subseq l1 l2) : subseq (h::l1) (h::l2).
...
Theorem subseq_app : forall (l1 l2 l3 : list nat),
  ...
  subseq l1 (l2 ++ l3).
Proof.
  intros l1 l2. generalize dependent l1. induction l2. 
  - intros. inversion H. apply q1.
  - intros. inversion H.
    + apply q1.
    + simpl. apply q2. apply IHl2. apply H2.
    + simpl. apply q3. apply IHl2. apply H2.
Qed.
(* xu yao e wai ding li
destruct l1.
    + apply q1.
    + destruct (x =? x0) eqn:E.
      ++ simpl. apply eqb_eq in E. rewrite E. rewrite E in H.
         apply q3. apply IHl2. 
*)
Module Draft.
Lemma HH : forall (n:nat) (l1 l2 : list nat),
subseq l1 l2 -> subseq l1 (n::l2).
Proof.
  intros. destruct l2.
  - inversion H. apply q1.
  - apply q2. apply H.
Qed. 
End Draft.
...
Proof.
  ...
  - intros. inversion H. apply q1. 
  - intros. apply q2. apply IHsubseq. apply H.
  - intros. inversion H.
    + apply q1.
    + apply q2. apply IHsubseq. apply H3.
    + apply q3. apply IHsubseq. apply H3.
Qed.
...
Proof.
 ...
 (*apply (MApp [1] (Char 1) [2;3] (App(Char 2)(App (Char 3) EmptyStr))).*)
  apply (MApp [1]).
  { apply MChar. }
  apply (MApp [2]).
  { apply MChar. }
...
Lemma MStar' : forall T (ss : list (list T)) (re : reg_exp),
  ...
  fold app ss [] =~ Star re.
Proof.
  intros. Check@ fold. induction ss.
  - simpl. apply MStar0.
...
Proof.
  ...
  - intros. generalize dependent s1. induction s2.
    ...
    + intros. simpl. inversion H.
      assert (HH : forall TT (x: TT) (y: list TT), x::y = [x]++y).
      {intros. simpl. reflexivity. } 
      rewrite HH. apply MApp.
      ++ apply MChar.
...
Proof.
  ...
  (* WORKED IN CLASS *)
  - (* MEmpty *)inversion Hin.
    (*apply Hin.*)
  - (* MChar *)simpl. inversion Hin. + left. apply H. + inversion H.
    (*apply Hin.*)
...
Proof.
  ...
  - intros. induction re.
    ...
    + simpl in H. apply andb_true_iff in H. destruct H as [H1 H2].
      apply IHre1 in H1. apply IHre2 in H2. destruct H1 as [s1 H1].
      destruct H2 as [s2 H2]. exists (s1++s2).
      apply MApp. 
      ++ apply H1.
      ++ apply H2.
...
Proof.
  ...
  + inversion Heqre'. rewrite H2 in *.
    destruct IHexp_match2 as [ss [L R]]. 
    - reflexivity.
    - exists (s1::ss). split. 
      * simpl. rewrite <- L. reflexivity.
      * intros. destruct H1 as [LL|RR].
        ** rewrite <- LL. apply H.
        ** apply R. apply RR.
Qed.
...
Proof.
  ...
  - simpl. intros. rewrite app_length in H.
    assert (HH: forall a b c d:nat, a+b<=c+d -> a<=c \/ b<=d).
    {intros. omega. }
    apply HH in H. destruct H as [L|R].
    -- apply IH1 in L. destruct L as [s2' [s3' [s4' [H1 [H2 H3]]]]].
      exists s2', s3', (s4' ++ s2). split.
      + rewrite H1. do 3 rewrite app_assoc. reflexivity.
      + split. 
        ++ apply H2.
        ++ intros. do 2 rewrite app_assoc. apply MApp. 
           do 1 rewrite <- app_assoc. apply H3. apply Hmatch2.
    -- apply IH2 in R. destruct R as [s2' [s3' [s4' [H1 [H2 H3]]]]].
      exists (s1++s2'), s3', s4'. split.
      + rewrite H1. do 2 rewrite app_assoc. reflexivity.
      + split. 
        ++ apply H2.
        ++ intros. do 1 rewrite <- app_assoc. apply MApp. 
           apply Hmatch1. apply H3.
  - simpl. intros. 
    assert (HH: forall a b c:nat, a+b<=c -> a<=c).
    {intros. omega. }
    apply HH in H. apply IH in H. 
  ...
  - simpl. intros. 
    assert (HH: forall a b c:nat, a+b<=c -> b<=c).
    {intros. omega. }
    apply HH in H. apply IH in H. 
    destruct H as [s2' [s3' [s4' [H1 [H2 H3]]]]].
    exists s2', s3', s4'. split.
  ...
  - simpl. intros. rewrite app_length in H.
    ...
    -- simpl in H.
       assert (HH: forall (X:Type) (l:list X), length l = 0 -> l = []).
       {intros. destruct l. - simpl. reflexivity. - inversion H0. }
       apply HH in E.
       rewrite E. simpl. simpl in IH2. apply IH2 in H.
       destruct H as [s2' [s3' [s4' [H1 [H2 H3]]]]].
       exists s2', s3', s4'. split.
       + apply H1.
       + split. apply H2. apply H3.
    -- simpl in IH2. clear IH1. clear IH2. 
       exists [], s1, s2.
       split.
       + reflexivity. 
       + split.
         ++ destruct s1. inversion E. intros HH. inversion HH.
         ++ simpl. induction m. 
            * simpl. apply Hmatch2.
...
Proof.
  ...
  - inversion H0.
  - destruct (eqbP n x) as [HH|HH].
    + rewrite HH in *. simpl in H. rewrite <- eqb_refl in H.
      inversion H. 
    + simpl in H. apply IHl. intros.
      * apply and_exercise in H. destruct H as [L R]. apply R.
      * simpl in H0. destruct H0 as [L|R].
        ** symmetry in L. apply HH in L. destruct L.
        ** apply R.
Qed.
...
| n1 : nostutter nil
| n2 x : nostutter [x]
| n3 x y l : x <> y -> nostutter(y::l) -> nostutter (x::y::l)
.
...
| m1 : Merge nil nil nil
| m2 x l1 l2 l3: Merge l1 l2 l3 -> Merge (x::l1) l2 (x::l3)
| m3 x l1 l2 l3: Merge l1 l2 l3 -> Merge l1 (x::l2) (x::l3).

Theorem in_order_merge : 
forall (X: Type) (test: X->bool) (l l1 l2: list X),
Merge l1 l2 l -> 
(forall a, In a l1 -> test a = true) ->
(forall b, In b l2 -> test b = false) -> 
filter test l = l1.
Proof.
  intros. induction H.
  - simpl. reflexivity.
  - simpl. destruct (test x) eqn:E.
    + assert (HH: forall (X': Type) (x': X') (l1' l2': list X'), 
      l1' = l2' -> x'::l1' = x':: l2'). {intros. 
      rewrite H2. reflexivity. }
      apply HH. apply IHMerge.
      ++ simpl in H0. intros. apply H0. right. apply H2.
      ++ apply H1. 
    + assert (HH: In x (x::l1)). {simpl. left. reflexivity. }
      apply H0 in HH. rewrite E in HH. discriminate HH.
  - simpl. destruct (test x) eqn:E.
    + assert (HH: In x (x::l2)). {simpl. left. reflexivity. }
      apply H1 in HH. rewrite E in HH. discriminate HH.
    + apply IHMerge.
      ++ apply H0.
      ++ simpl in H1. intros. apply H1. right. apply H2.
Qed.
...
Theorem Longest : 
forall  (test: nat->bool) (l l': list nat) (x: nat),
subseq l' l ->
(forall x, In x l' -> test x = true) -> 
length l' <= length (filter test l).
Proof. (* to make life easier, i chooose not to change the
definition of subseq but using nat in this exercise*)
  assert (HH: forall (ll: list nat),
    0 <= length ll). { intros. induction ll. - simpl. apply O_le_n.
    - simpl. apply O_le_n. }
  assert (HHH: forall (a b: nat), a <= b -> a <= S b).
  { intros. apply (le_trans a b (S b)). apply H. apply le_S.
    apply le_n. }
  intros. induction H.
  - simpl. Search length. 
    apply HH.
  - simpl. destruct (test h).
    + simpl.
      apply HHH. apply IHsubseq. apply H0.
    + apply IHsubseq. apply H0.
  - simpl. destruct (test h) eqn:E.
    + simpl. apply n_le_m__Sn_le_Sm. apply IHsubseq. 
      simpl in H0. intros. apply H0. right. apply H1.
    + simpl in H0. assert (E': test h = true).
...
| p1 : pal nil
| p2 x : pal [x] 
| p3 x l : pal l-> pal (x::l++[x]).

Theorem pal_app_rev : 
forall l : list nat , pal (l ++ rev l).
Proof.
  intros. induction l.
  - simpl. apply p1.
...
Theorem pal_rev_that :
forall l: list nat, pal l -> l = rev l.
Proof.
  intros. 
  induction H.
  - auto.
  - auto.
  - simpl. rewrite (rev_app_distr nat l [x]).
...
Lemma sim2 :
forall (l1 l2: list nat) (x1 x2: nat),
[x1]++l1 = [x2]++l2 -> l1 = l2 /\ x1 = x2.
Proof.
  Admitted.*)

(* credit to https://stackoverflow.com/questions/
43011411/how-to-do-induction-differently *)

Axiom nat_ind2 : forall P : nat -> Prop, P 0 -> P 1 ->
  (forall n : nat, P n -> P (S (S n))) -> forall n : nat, P n.
About length.

Lemma lel_nil : forall l':list nat,
  length l' = 0 -> nil = l'.
Proof. 
...
Qed.
(*zhijie yongle biaozhunku*)

Theorem length_zero_iff_nil (l : list nat):
   length l = 0 <-> l = nil.
Proof. 
  split.
  - intros. apply lel_nil in H. symmetry. apply H.
  - intros. rewrite H. auto.
Qed.

Lemma list_ind2 : forall (P : list nat -> Prop) (P_nil : P [])
...
Proof.
  ...
  - destruct l; [discriminate|]. simpl in Heqn.
    inversion Heqn; subst. pose proof (rev_involutive nat l) as Hinv.
...
Qed.
  ...
  (* // it is better to use induction with step length 2.
    intros. induction l.
  - apply p1.
  - simpl in H. induction IHl.
    + apply p2.
    + simpl in H. inversion H.
      assert (HH: [x0 ; x0] = (x0::nil ++ [x0]) ).
      { auto. } rewrite HH. apply p3. apply p1.
    + simpl in H. 
      apply (sim1 (x :: x0 :: l) (rev (l ++ [x0]) ++ [x0]) x0 x) in H.
      destruct H as [L R].
      rewrite R in *. assert (HH: x ::( x :: l ++ [x]) = x :: (x::l) ++ [x]).
      { auto. } rewrite HH. apply p3. apply IHp.
      rewrite rev_app_distr in L. simpl in L. apply sim2 in L.
      destruct L as [LL LR]. apply LL.
    + 
  - Abort. *)

Theorem more_diffcult :
forall l: list nat, l = rev l -> pal l.
Proof.
  intros. induction l using list_ind2.
  - apply p1.
  - apply p2.
  - simpl in H. rewrite rev_app_distr in H. 
    rewrite <- app_assoc in H. simpl in H. inversion H.
    apply p3. apply IHl. assert (HH: forall (l1 l2: list nat),
    l1 = l2 -> rev l1 = rev l2). { intros. rewrite H0. auto. }
    apply HH in H2. do 2 rewrite rev_app_distr in H2.
    simpl in H2. inversion H2. rewrite rev_involutive in H3.
    auto.
Qed.
(* FILL IN HERE 
...
| d1 : disjoint nil nil
| d2 x l1 l2 : ~In x l2 -> disjoint l1 l2 -> disjoint (x::l1) l2
| d3 x l1 l2 : ~In x l1 -> disjoint l1 l2 -> disjoint l1 (x::l2).

Inductive NoDup {X: Type} : list X -> Prop :=
| nodup1 : NoDup nil
| nodup2 x l : ~In x l -> NoDup l -> NoDup (x::l).

Theorem interesting :
NoDup [4] /\ disjoint nil [4].
Proof.
  split.
  - apply nodup2.
...
Proof.
  intros. induction l.
  - inversion H.
  - intros. simpl in H. destruct H as [L|R].
    + rewrite L in *. exists nil, l. auto.
    + apply IHl in R. destruct R as [l5 [l6 H]]. 
      exists (x0::l5), l6. rewrite H. auto.
Qed.
...
| re1 x l : In x l -> repeats (x::l)
| re2 x l : repeats l -> repeats (x::l).
(*actually we can combine it using \/ *)
...
Lemma final_lemma : forall (X:Type) (x x1:X) (k1 k2: list X),
In x (k1 ++ (x1 :: k2)) <-> In x (x1::k1 ++ k2).
Proof. 
About LF.Logic.In.
    intros. rewrite In_app_iff. simpl. rewrite In_app_iff.
    rewrite or_comm. rewrite (or_comm (In x k1) (In x k2)).
    rewrite or_assoc. apply iff_refl.
Qed.
...
Proof.
  ...
  - intros. destruct l2 as [| x2 l2].
    + simpl in H0. destruct (H0 x1). left. reflexivity.
    + remember H as H'. unfold excluded_middle in H. 
      destruct (H (In x1 l1)) as [L|R].
      ++ apply re1. apply L.
      ++ assert (temp: In x1 (x1 :: l1)). { simpl. left. auto. }
         apply H0 in temp. About in_split. apply in_split in temp.
         destruct temp as [lnew [lnew' Hnew]].
          
         apply re2. apply (IHl1' (lnew++lnew')).
         * apply H'.
         * intros. simpl in H0. assert (temp1: In x (x2::l2)).
              { simpl. apply H0. right. auto. } rewrite Hnew in temp1. 
           rewrite final_lemma in temp1. simpl in temp1.
           destruct temp1 as [LL|RR].
           -- rewrite LL in R. apply R in H2. destruct H2.
           -- apply RR.
         * simpl in H1. assert (lengthwise: length(x2::l2) = 
           length (lnew ++ x1 :: lnew')). { rewrite Hnew. auto. }
           rewrite app_length in lengthwise. simpl in lengthwise. 
            rewrite <- plus_n_Sm in lengthwise. injection lengthwise.
            intros le'. rewrite app_length. rewrite <- le'.
            unfold lt in H1. unfold lt. apply Sn_le_Sm__n_le_m in H1.
            apply H1.
Qed.

(*remember H as H'. unfold excluded_middle in H. 
    destruct (H (In x l1')) as [L|R].
    + apply re1. apply L.
    + apply re2. apply (IHl1' l2). 
      ++ apply H'. 
...
Proof.
  ...
  - intros. apply app_exists in H. 
    destruct H as [s0 [s1 [H1 [H2 H3]]]]. destruct s0.
    + left. split. 
      ++ apply H2.
      ++ simpl in H1. rewrite H1. apply H3.
    + right. inversion H1. exists s0, s1. split.
      ++ auto.
      ++ split. * apply H2. * apply H3.
  - intros. destruct H as [[H1 H2]|[s0[s1[H1[H2 H3]]]]].
    + assert (temp: a::s = []++a::s). { auto. } rewrite temp. 
      apply MApp. ++ apply H1. ++ apply H2.
    + rewrite H1. assert (temp: a::s0 ++s1 = (a::s0)++s1). { auto. } 
      rewrite temp. apply MApp. ++ apply H2. ++ apply H3.
Qed.
...
Proof.
  ...
  - intros. remember (a::s) as s'. remember (Star re) as re'. induction H.
    ...
    + inversion Heqre'.
    + inversion Heqs'.
    + inversion Heqre'. rewrite H2 in *. destruct s1.
      * simpl in Heqs'. apply IHexp_match2. apply Heqs'. auto.
      * inversion Heqs'. (*rewrite H3 in *. rewrite <- H4 in *.*) 
        exists s1, s2. split. auto. split. rewrite <- H3. 
        apply H. apply H0.
  - intros. destruct H as [s0 [s1 [H1 [H2 H3]]]].
    rewrite H1. assert (HH: a::s0++s1=(a::s0)++s1). { auto. }
    rewrite HH. apply MStarApp. 
    + apply H2.
    + apply H3.
Qed.
...
| EmptyStr => true
| Char _ => false
| App re3 re4  => (match_eps re3) && (match_eps re4)
| Union re3 re4 => (match_eps re3) || (match_eps re4)
| Star re => true
end.
...
Proof.
  unfold refl_matches_eps. Search reflect.
  induction re.
  - simpl. apply ReflectF. unfold not. intros. inversion H.
  - simpl. apply ReflectT. apply MEmpty.
  - simpl. apply ReflectF. unfold not. intros. inversion H.
  - simpl. apply iff_reflect. rewrite andb_true_iff. 
    apply reflect_iff in IHre1. apply reflect_iff in IHre2.
    split. 
    + intros. inversion H. 
      assert (temp: s1 ++ s2 = [ ] -> s1 = [] /\ s2 = []).
      { intros. destruct s1. simpl in H5. rewrite H5. auto.
        inversion H5. }
      apply temp in H1. destruct H1 as [L R].
      rewrite L in *. rewrite R in *.
      split.
      ++ rewrite <- IHre1. apply H3.
      ++ rewrite <- IHre2. apply H4.
    + intros. destruct H as [L R]. assert (H: @nil ascii = [] ++ []).
      { auto. } rewrite H.
      apply MApp. apply IHre1. apply L. apply IHre2. apply R.
  - simpl. apply iff_reflect. rewrite orb_true_iff. 
    apply reflect_iff in IHre1. apply reflect_iff in IHre2.
    split.
    + intros. inversion H. 
      ++ left. apply IHre1. apply H2.
      ++ right. apply IHre2. apply H1.
    + intros. destruct H as [L|R].
...
| App re3 re4  => 
  if (match_eps re3) then Union (derive a re4) (App (derive a re3) re4)  
                    else App (derive a re3) re4
| Union re3 re4 => Union (derive a re3) (derive a re4)
...
Proof.
  ...
  - intros. unfold is_der. unfold is_der in IHre1. unfold is_der in IHre2. split.
    ...
    + simpl. destruct (match_eps re3) eqn:eq. 
      * intros. apply union_disj in H. inversion H. 
        { apply app_ne. apply IHre2 in H0. left.
          split. apply (reflect_iff _ (match_eps re3)). apply match_eps_refl. apply eq. apply H0. }
...
(*  unfold derives. unfold is_der. intros. 
  generalize dependent a. induction re.
  - simpl. split.
    + intros. inversion H.
    + intros. inversion H.
  - simpl. split.
    ...
    + intros. inversion H.
  - simpl. split.
    + intros. destruct (eqb a t) eqn:E.
      * simpl. inversion H. rewrite eqb_eq in E. apply MEmpty.
      * rewrite eqb_neq in E. inversion H. apply E in H3. destruct H3.
    + intros. destruct (eqb a t) eqn:E.
      * simpl. rewrite eqb_eq in E. inversion H. rewrite E. apply MChar.
      * rewrite eqb_neq in E. inversion H.
  - split.
    + intros. apply app_ne in H. destruct H as [[H1 H2]|[s0 [s1 [H1 [H2 H3]]]]]. 
      * rewrite (reflect_iff _ (match_eps re3)) in H1.
        ** simpl. rewrite H1. apply MUnionR. apply IHre2 in H2. apply H2.
        ** simpl. apply match_eps_refl.
      * rewrite H1. destruct (match_eps re3) eqn:eq.
        { simpl. rewrite eq. apply MUnionL. apply MApp. apply IHre2 in H2. apply H2. apply H3. }
        { simpl. rewrite eq. apply MApp. apply IHre1 in H2. apply H2. apply H3. }*)
(** [] *)
