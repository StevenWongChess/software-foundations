...
Definition member (v:nat) (s:bag) : bool := negb (O =? (count v s)).
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
  | nil => true
  | s::t => match (member s s2) with
            | true => (subset t (remove_one s s2))
            | false => false
            end
end.
...
Lemma beq_refl_formal : forall n:nat, 
true = (n=?n).
Proof.
...
Proof.
  intros n s.
  simpl. rewrite <- beq_refl_formal. reflexivity.
Qed.
...
Proof.
  ...
  - (* l = nil *)
    reflexivity.
  - (* l = cons *)
    simpl. rewrite -> app_length. simpl. rewrite -> plus_comm.
...
Proof.
  ...
  - simpl. reflexivity.
  - destruct h. + simpl. rewrite -> IHt. reflexivity. 
...
Proof.
  ...
  - simpl. reflexivity.
  - destruct h as [|h']. + simpl. rewrite <- IHt. reflexivity.
                         + simpl. rewrite <- IHt. 
                           rewrite <- (eqb_refl h'). reflexivity.
Qed.
...
Proof.
  intros l1 l2 H.
  assert (HH : rev (rev l1) = rev (rev l2)).
  {rewrite -> H. reflexivity. }
  do 2 rewrite -> rev_involutive in HH.
